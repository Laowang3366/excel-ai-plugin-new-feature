...
- **当前测试源基线**：静态统计为 128 个测试文件、683 个 `it/test` 用例；具体通过情况以每项修复记录中的 `vitest`、`typecheck` 和 `build` 验证命令为准。
...
- **常规设置字段复用**：抽出通用开关与滑块设置字段，`GeneralSettings.tsx` 降至 400 行以内，同时保留透明度、动态数组、压缩和数据目录设置链路不变。
