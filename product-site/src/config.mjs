...
export function loadConfig(overrides = {}) {
  ...
  const config = {
    ...
    dataDir,
    releasesDir: path.resolve(
      overrides.releasesDir ||
        process.env.RELEASES_DIR ||
        path.join(root, ".local", "releases"),
    ),
    databasePath: path.resolve(
      overrides.databasePath ||
        process.env.DATABASE_PATH ||
        path.join(dataDir, "analytics.sqlite"),
    ),
    adminPasswordHash:
      overrides.adminPasswordHash ||
      requiredInProduction(
        "ADMIN_PASSWORD_HASH",
        "scrypt$YpRPhq5bCIWtwFLGCwBvZQ==$rPJ4dulEcgeS+t95RvP8Hv6ZDDtgZu47u19j7boQHVI=",
      ),
    cookieSecret:
      overrides.cookieSecret ||
      requiredInProduction(
        "COOKIE_SECRET",
        "development-cookie-secret-change-before-deploy-1234",
      ),
    analyticsSalt:
      overrides.analyticsSalt ||
      requiredInProduction("ANALYTICS_SALT", "development-analytics-salt"),
    analyticsRetentionDays: readBoundedInteger(
      "ANALYTICS_RETENTION_DAYS",
      overrides.analyticsRetentionDays,
      90,
      1,
      3650,
    ),
    analyticsIpRotationDays: readBoundedInteger(
      "ANALYTICS_IP_ROTATION_DAYS",
      overrides.analyticsIpRotationDays,
      30,
      1,
      365,
    ),
    useAccelRedirect:
      overrides.useAccelRedirect ?? process.env.USE_ACCEL_REDIRECT === "true",
  };
  ...
  if (
    ...
    !/^scrypt\$[^$]+\$[^$]+$/.test(config.adminPasswordHash)
  ) {
