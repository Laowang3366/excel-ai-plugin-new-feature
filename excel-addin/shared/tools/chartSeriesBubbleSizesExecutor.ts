...
export async function executeChartSeriesBubbleSizesTool(
  host: HostAdapter,
  call: ToolCall,
): Promise<ToolResult | null> {
  if (call.name !== "chart.series.bubbleSizes.update") return null;
  rejectUnknown(call.arguments, ["sheetName", "chartName", "seriesIndex", "bubbleSizesRange"]);
  return fromHost(
    call.name,
    await host.updateChartSeriesBubbleSizes({
      ...
      bubbleSizesRange: requireString(call.arguments, "bubbleSizesRange"),
    }),
  );
}
