...
- 仅操作当前宿主已打开的工作簿：先 `host.status`，再 `workbook.inspect` / `workbook.save`（需保存时）/ `selection.get` / `sheet.list` 了解结构。
...
- Power Query、透视表、切片器、任意路径 open/create/saveAs/switch 工作簿（当前簿可用 `workbook.save` 原地保存，无路径参数）。
