...
export function buildAddInHardBoundary(): string {
  return [
    ...
    "- Power Query / 切片器：**unsupported**，不得伪造成功。Office.js 可用 `pivot.list`/`pivot.create`（ExcelApi 1.8）与 `pivot.refresh`（ExcelApi 1.3，当前工作簿；需 advancedIntent=interactive-pivot）；`refreshConnections=true` 为有限 `dataConnections.refreshAll`（非完整 RefreshAll）；WPS 透视表 **typed unsupported**；不得伪造成功。",
...
export function buildAdvancedExcelBoundary(context: PromptRoutingContext = {}): string {
  ...
  const rules = [
    ...
    "透视表用 `pivot.list`/`pivot.create`（Office.js ExcelApi 1.8）/`pivot.refresh`（ExcelApi 1.3；create/refresh 需 advancedIntent=interactive-pivot；≥1 字段；空 destination→Pivots lastBottom+3；`refreshConnections=true`→ExcelApi 1.7 dataConnections.refreshAll（有限连接、verified:false）；切片器 unsupported；WPS unsupported）。对象清单优先 `workbook.objects.inspect`（safe；按分类 available|unsupported|failed；maxItemsPerCategory 截断且保留 totalCount），避免连续 list 膨胀上下文。",
  ...
  if (intents.has("interactive-pivot")) {
    rules.push(
      "本轮检测到交互式透视意图：Office.js 使用 `pivot.list`（safe）/`pivot.create`/`pivot.refresh`（需 advancedIntent=interactive-pivot；写后回读；destination 空→Pivots 表；refreshConnections=true→有限 dataConnections.refreshAll（verified:false，非完整 RefreshAll）；切片器仍 unsupported）；WPS 透视表 typed unsupported，不得伪造成功。普通汇总仍用 range/table，勿无故升级为透视表。",
    );
  }
