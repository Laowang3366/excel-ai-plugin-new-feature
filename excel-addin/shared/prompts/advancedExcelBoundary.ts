...
export function buildAddInHardBoundary(): string {
  return [
    ...
    "- Power Query / 切片器：**unsupported**，不得伪造成功。Office.js 可用 `pivot.list`/`pivot.create`/`pivot.refresh`（ExcelApi 1.8，当前工作簿；需 advancedIntent=interactive-pivot）；WPS 透视表 **typed unsupported**；不得伪造成功。",
...
export function buildAdvancedExcelBoundary(context: PromptRoutingContext = {}): string {
  ...
  if (intents.has("interactive-pivot")) {
    rules.push(
      "本轮检测到交互式透视意图：Office.js 使用 `pivot.list`（safe）/`pivot.create`/`pivot.refresh`（需 advancedIntent=interactive-pivot；写后回读；destination 空→Pivots 表；refreshConnections=true unsupported；切片器仍 unsupported）；WPS 透视表 typed unsupported，不得伪造成功。普通汇总仍用 range/table，勿无故升级为透视表。",
    );
  }
