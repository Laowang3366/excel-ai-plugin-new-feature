...
- **Phase53/53.1** `errorAlert`/`prompt` are DataValidation **top-level metadata** (not rule fields). ClientObject contract: load top-level `type,rule,ignoreBlanks,errorAlert,prompt` → sync → merge request onto loaded snapshot → **whole-object assign** `dv.errorAlert`/`dv.prompt` (not nested mutation) → write sync → reload. Official styles: stop|warning|information → host Stop|Warning|Information. title/message maxLength 255; empty `{}` rejected. Omit metadata preserves host. Incomplete host snapshot → ordinary failed zero writes. Requirement ExcelApi **1.8**. Inline listValues maxItems 1000 and serialized source ≤255 chars.
