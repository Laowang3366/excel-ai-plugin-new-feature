...
- **Phase53** `errorAlert`/`prompt` are DataValidation **top-level metadata** (not rule fields). Official styles: stop|warning|information → host Stop|Warning|Information. title/message maxLength 255. Write only applies metadata fields present in the request (omit preserves host). Read returns full host snapshot. Requirement ExcelApi **1.8**. Inline listValues maxItems 1000 and serialized source ≤255 chars.
