...
| chart | series trendlines format update | implemented | **unsupported** | formatChart trendline format | Office.js `ChartTrendline.format.line` **ExcelApi 1.7** (`color`/`lineStyle`/`weight`); 1-based seriesIndex+trendlineIndex; write→sync→load→sync; **unsupported**: label styling/font/fill, clear(); WPS unsupported; **not** real sideload verified |
...
- Phase49: `chart.series.trendlines.format.update` (format.line color/lineStyle/weight ExcelApi 1.7; Office.js; WPS unsupported; **not** real sideload verified)
