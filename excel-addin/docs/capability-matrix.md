...
| pivot | list/create/refresh (current workbook) | implemented | **unsupported** | `pivot.list` / `pivot.create` / `pivot.refresh` | Office.js **ExcelApi 1.8** `Worksheet.pivotTables.add` + row/column/filter/data hierarchies; create post-write hierarchy count verify; empty destination → `Pivots` sheet auto address; `refreshConnections` **unsupported** (no Workbook.RefreshAll); slicers **unsupported**; WPS typed unsupported; **not** real Excel sideload verified |
