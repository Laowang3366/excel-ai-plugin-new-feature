...
| `sheet.pageLayout.get/set` | Office.js：**ExcelApi 1.9** `pageLayout` 子集（orientation/margins top|bottom|left|right|header|footer points/**headers|footers default 六槽文本**（""清除）/print flags/zoomScale/paperSize a3|a4|a5|letter|legal/fitToPagesWide|Tall 1..32767/draft/pageOrder downThenOver|overThenDown/firstPageNumber≥1/printArea/titles）；fit 与 zoomScale 互斥；print* 仅非空设置。**无** clear/first|even|odd pages/headersFooters state/图片/page breaks/fitToOnePage。WPS → unsupported |
