...
export interface ProviderTemplate {
  ...
  defaultModel: string;
  apiFormat: string;
  /** 直连供应商预设模型列表；聚合供应商不设此字段，由用户自定义输入 */
  presetModels?: string[];
  /** 默认上下文窗口大小（tokens），选择模板时自动填入 */
  defaultContextWindowSize: number;
  /** 供应商分类 */
  category: ProviderCategory;
  /** 可选的思考等级选项（>2 为等级型下拉框，===2 为开关） */
...
/** 等级型：off / 低 / 中 / 高 / 极高 (OpenAI, Anthropic) */
...
/** 等级型：off / 高 / 极高 (DeepSeek, 智谱 — low/medium 映射为 high) */
