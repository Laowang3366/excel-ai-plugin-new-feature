...
export const AttachmentImagePreview: React.FC<AttachmentImagePreviewProps> = React.memo(({
  ...
  useEffect(() => {
    ...
    ipcApi.file.readAsBase64(attachment.filePath)
      .then((result) => {
        if (cancelled) return;
  ...
  const lightbox = lightboxOpen && canPreview && typeof document !== "undefined"
    ? createPortal(
      <div className="image-preview-lightbox" onClick={() => setLightboxOpen(false)}>
        <div className="image-preview-panel" onClick={(event) => event.stopPropagation()}>
          <div className="image-preview-toolbar">
            <span className="image-preview-title" title={attachment.fileName}>{attachment.fileName}</span>
  ...
  return (
    <div className={`image-attachment-preview ${variant}-image-preview ${preview.status}`}>
      ...
      <div className="image-attachment-info">
        <span className="image-attachment-name" title={attachment.fileName}>{attachment.fileName}</span>
