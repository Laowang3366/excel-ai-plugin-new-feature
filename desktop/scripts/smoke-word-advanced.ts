  ...
  try {
    ...
    const actions: OfficeActionInput[] = [
      ...
      },
      {
        ...
        filePath: sourcePath,
        params: { command: "addBookmark", name: "Overview", start: 0, end: 3 },
      },
      {
        app: "word",
        action: "insert",
        operation: "manageReferences",
        filePath: sourcePath,
        params: { command: "addFootnote", text: "脚注来源" },
      },
      {
        app: "word",
        action: "insert",
        operation: "manageReferences",
        filePath: sourcePath,
        params: { command: "addEndnote", text: "尾注来源" },
      },
      {
        app: "word",
        action: "insert",
        operation: "manageReferences",
        filePath: sourcePath,
        params: { command: "addCaption", targetType: "range", label: "图", title: "测试题注" },
      },
      {
        app: "word",
        action: "insert",
        operation: "manageReferences",
        filePath: sourcePath,
        params: { command: "addCrossReference", referenceType: "bookmark", item: "Overview" },
      },
      {
        app: "word",
        action: "insert",
        operation: "manageReferences",
        filePath: sourcePath,
        params: { command: "addTableOfFigures", label: "图" },
      },
      { app: "word", action: "inspect", operation: "inspectReferences", filePath: sourcePath },
      {
        ...
        filePath: sourcePath,
        params: {
          edits: [{ command: "replace", find: "旧内容", replace: "新内容", all: true }],
          keepTracking: true,
        },
      },
      { app: "word", action: "inspect", operation: "inspectRevisions", filePath: sourcePath },
      {
        ...
        params: { revisedFilePath: revisedPath, author: "Smoke Compare" },
      },
      {
        app: "word",
        action: "edit",
      ...
      },
      {
        ...
        params: {
          values: {
            name: "张三",
            approved: true,
            date: { value: "2026-07-12", dateFormat: "yyyy-MM-dd" },
            department: "rd",
            photo: { value: imagePath, kind: "image", width: 36 },
          },
    ...
    for (const action of selectedActions) {
      ...
      if (keepArtifacts)
        await writeFile(
          path.join(tempDir, `${action.operation}${command}.json`),
          JSON.stringify(action, null, 2),
          "utf8",
        );
      const result = await bridge.executeAction(action);
      results.push({
        operation: `${action.operation}${command}`,
        status: result.status,
        error: result.error,
      });
      if (result.status !== "done")
        throw new Error(`${action.operation}${command}: ${result.error || result.summary}`);
      verifyResult(action, result.data);
    }
    if (selectedActions.some((action) => action.operation === "compareDocuments"))
      await access(comparisonPath);
    if (selectedActions.some((action) => action.operation === "mailMerge"))
      await access(singleMergePath);
    ...
    if (selectedActions.some((action) => action.operation === "batchMailMerge")) {
      ...
      if (docxCount !== 2 || pdfCount !== 2)
        throw new Error(`批量邮件合并输出数量错误: docx=${docxCount}, pdf=${pdfCount}`);
    }
...
function verifyResult(action: OfficeActionInput, data: unknown): void {
  ...
  if (action.operation === "inspectReferences") {
    ...
    if (
      !Array.isArray(references.bookmarks) ||
      references.bookmarks.length < 1 ||
      Number(references.footnoteCount) < 1 ||
      Number(references.endnoteCount) < 1
    ) {
  ...
  if (action.operation === "inspectRevisions") {
    ...
    if (Number(review.revisionCount) < 1 || review.trackRevisions !== true)
      throw new Error(`修订检查失败: ${JSON.stringify(review)}`);
  }
  if (action.operation === "compareDocuments") {
    const summary = operationData;
    if (Number(summary.revisionCount) < 1)
      throw new Error(`文档对比失败: ${JSON.stringify(summary)}`);
  }
  if (action.operation === "inspectContentControls") {
    const controls = Array.isArray(operationData.controls)
      ? operationData.controls.map(asRecord)
      : [];
    if (controls.length !== 5) throw new Error(`内容控件数量错误: ${controls.length}`);
    if (!controls.some((control) => control.tag === "approved" && control.checked === true))
      throw new Error("复选框内容控件填充失败");
    if (
      !controls.some(
        (control) => control.tag === "department" && String(control.text).includes("研发"),
      )
    )
      throw new Error("下拉内容控件填充失败");
  }
...
async function createFixtures(input: {
  ...
  const results = await Promise.all([
    applyWordAdvancedAction({
      ...
      params: {
        title: "一、项目概览",
        paragraphs: ["这是旧内容，用于修订测试。", "（一）实施范围", "正文段落。"],
      },
    }),
    applyWordAdvancedAction({
      operation: "createDocument",
      filePath: input.revisedPath,
      params: {
        title: "一、项目概览",
        paragraphs: ["这是修订后的内容。", "（一）实施范围", "新增段落。"],
      },
    }),
    applyWordAdvancedAction({
      operation: "createDocument",
      filePath: input.controlPath,
      params: { title: "智能模板", paragraphs: ["以下字段由内容控件填充。"] },
    }),
    applyWordAdvancedAction({
      operation: "createDocument",
      filePath: input.mergeTemplatePath,
      params: {
        title: "合同通知",
        paragraphs: ["客户：{{Name}}", "编号：{{Id}}", "状态：{{StatusText}}", "照片：{{Photo}}"],
      },
    }),
    applyExcelAdvancedAction({
      operation: "createWorkbook",
      filePath: input.dataPath,
      params: {
        values: [
          ["Name", "Id", "Status", "Photo"],
          ["张三", "A001", "active", input.imagePath],
          ["李四", "A002", "pending", input.imagePath],
        ],
      },
    }),
  ]);
