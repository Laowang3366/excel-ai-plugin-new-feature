...
async function main(): Promise<void> {
  ...
  try {
    ...
    await run(bridge, {
      ...
      filePath: sourcePath,
      outputPath: wordPath,
      target: "range:Sheet1!A1:B3",
      params: { linked: true, linkId, overwrite: true, sourceHost: "excel", wordHost: "word" },
    });
    await run(bridge, {
      app: "word",
      action: "insert",
    ...
    const update = await run(bridge, {
      ...
      outputPath: wordPath,
      target: "range:Sheet1!A1:B3",
      params: { linked: true, updateExisting: true, linkId, sourceHost: "excel", wordHost: "word" },
    });
    assertBookmark(
      await run(bridge, {
        app: "word",
...
async function run(
  ...
  if (result.status !== "done")
    throw new Error(`${input.operation}: ${result.error || result.summary}`);
  process.stdout.write(`Passed ${input.app}/${input.operation}\n`);
  return result;
}

function assertBookmark(result: OfficeActionResult): void {
  const bookmarks = Array.isArray(data(result).bookmarks)
    ? (data(result).bookmarks as Array<Record<string, unknown>>)
    : [];
