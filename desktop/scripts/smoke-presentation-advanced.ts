...
async function main(): Promise<void> {
  ...
  try {
    ...
    const actions: OfficeActionInput[] = [
      { app: "presentation", action: "inspect", operation: "inspectPresentationTheme", filePath: presentationPath },
      {
        app: "presentation",
        action: "style",
        ...
        params: {
          ...
          themeColors: [{ index: 5, value: "2563EB" }, { index: 6, value: "16A34A" }],
        },
      },
      { app: "presentation", action: "inspect", operation: "inspectPresentationTheme", filePath: presentationPath, params: { expectedThemeColors: [{ index: 5, value: "2563EB" }, { index: 6, value: "16A34A" }] } },
      { app: "presentation", action: "insert", operation: "insertChart", filePath: presentationPath, target: "slide:2", params: { name: "Revenue Chart", chartType: "column" } },
      {
        app: "presentation",
        action: "insert",
        ...
        filePath: presentationPath,
        target: "slide:2",
        params: { name: "KPI Table", values: [["指标", "结果"], ["收入增长", "18%"]], left: 80, top: 360, width: 520, height: 120 },
      },
      { app: "presentation", action: "insert", operation: "replacePictureSlot", filePath: presentationPath, target: "slide:2", params: { name: "Product Image", imagePath: logoPath, left: 640, top: 140, width: 120, height: 120, preserveAspectRatio: true } },
      { app: "presentation", action: "inspect", operation: "inspectSlideElements", filePath: presentationPath, params: { allSlides: true } },
      {
        app: "presentation",
        action: "style",
        ...
        params: {
          ...
          fitToSlide: true,
          edits: [
            { shapeName: "KPI Table", tableCells: [{ row: 2, column: 2, text: "20%", fontName: "Microsoft YaHei", fontSize: 16 }] },
            { shapeName: "Revenue Chart", chart: { title: "季度收入", hasLegend: false } },
            { shapeName: "Product Image", preserveAspectRatio: true, crop: { left: 0, right: 0, top: 0, bottom: 0 } },
          ],
        },
      },
      { app: "presentation", action: "inspect", operation: "inspectSlideElements", filePath: presentationPath, params: { allSlides: true } },
      {
        ...
        params: {
          ...
          effects: [
            ...
            { category: "path", effect: "appear", trigger: "afterPrevious", duration: 0.5, pathX: 0.08, pathY: 0 },
          ],
        },
      },
      { app: "presentation", action: "inspect", operation: "inspectAnimations", filePath: presentationPath, target: "slide:1" },
      {
        app: "presentation",
        action: "edit",
        operation: "configureSlideShow",
        filePath: presentationPath,
        params: { allSlides: true, showType: "speaker", autoPlay: true, loop: true, transition: "fade", advanceAfter: 3, transitionDuration: 0.5 },
      },
      {
        app: "presentation",
        action: "edit",
      ...
      { app: "presentation", action: "inspect", operation: "inspectSpeakerNotes", filePath: presentationPath, params: { allSlides: true } },
      { app: "presentation", action: "snapshot", operation: "exportHandouts", filePath: presentationPath, outputPath: handoutPath, params: { includeNotes: true } },
    ];
...
function verifyResult(action: OfficeActionInput, data: unknown): void {
  ...
  if (requestedHost === "powerpoint" && progId !== "powerpoint.application") throw new Error(`未使用 PowerPoint COM: ${operationData.progId || "unknown"}`);
  if (action.operation === "inspectPresentationTheme") {
    const theme = asRecord(operationData.theme);
    if (Number(theme.slideCount) < 2 || !Array.isArray(theme.designs) || theme.designs.length < 1) throw new Error("主题母版检查失败");
    const expectedColors = Array.isArray(action.params?.expectedThemeColors) ? action.params.expectedThemeColors.map(asRecord) : [];
    const colors = (theme.designs as unknown[]).flatMap((design) => Array.isArray(asRecord(design).colors) ? asRecord(design).colors as unknown[] : []).map(asRecord);
    for (const expected of expectedColors) {
      const expectedRgb = colorHexToOle(String(expected.value || ""));
      if (!colors.some((color) => Number(color.index) === Number(expected.index) && Number(color.rgb) === expectedRgb)) {
        throw new Error(`主题色回读失败: ${JSON.stringify(colors)}`);
      }
    }
  }
  if (action.operation === "applyMasterBranding" && Number(asRecord(operationData.updated).masters) < 1) throw new Error("母版品牌更新失败");
  if (action.operation === "applyMasterBranding") {
    if (operationData.appliedTemplate !== action.params?.templatePath) throw new Error("旧 PPT 模板转换失败");
    const designs = Array.isArray(asRecord(operationData.theme).designs) ? asRecord(operationData.theme).designs as unknown[] : [];
    const colors = designs.flatMap((design) => Array.isArray(asRecord(design).colors) ? asRecord(design).colors as unknown[] : []).map(asRecord);
    if (requestedHost === "wps") {
      if (operationData.themePackageFallback !== true) throw new Error("WPS 主题色未触发 Open XML 兼容更新");
    } else if (!colors.some((color) => Number(color.index) === 5 && Number(color.rgb) === 15426341)) {
  ...
  if (action.operation === "inspectSlideElements") {
    ...
    if (Number(summary.slideCount) < 2 || Number(summary.shapeCount) < 2) throw new Error(`元素检查失败: ${JSON.stringify(summary)}`);
    const slides = Array.isArray(operationData.slides) ? operationData.slides.map(asRecord) : [];
    const shapes = slides.flatMap((slide) => Array.isArray(slide.shapes) ? slide.shapes as unknown[] : []).map(asRecord);
    for (const typeName of ["chart", "table", "picture"]) {
      if (!shapes.some((shape) => shape.typeName === typeName)) throw new Error(`元素检查缺少 ${typeName}`);
    }
  }
  if (action.operation === "configureAnimations") {
    const snapshot = asRecord(operationData.snapshot);
    if (!Array.isArray(snapshot.effects) || snapshot.effects.length < 4) throw new Error("四类动画配置失败");
    const animated = Array.isArray(operationData.animated) ? operationData.animated.map(asRecord) : [];
    for (const category of ["entrance", "emphasis", "exit", "path"]) {
      if (!animated.some((item) => item.category === category)) throw new Error(`动画配置缺少 ${category}`);
    }
    const effects = snapshot.effects.map(asRecord);
    if (!effects.some((effect) => effect.exit === true)) throw new Error("退出动画未生效");
    if (!effects.some((effect) => Array.isArray(effect.behaviors) && effect.behaviors.map(asRecord).some((behavior) => Number(behavior.type) === 1 && Math.abs(Number(behavior.byX)) > 0))) throw new Error("路径动画行为未生效");
  }
  if (action.operation === "inspectAnimations") {
    const animations = Array.isArray(operationData.animations) ? operationData.animations.map(asRecord) : [];
    if (animations.length < 1 || !Array.isArray(animations[0].effects) || animations[0].effects.length < 4) throw new Error("动画检查失败");
  }
  if (action.operation === "configureSlideShow" && asRecord(operationData.slideShow).loop !== true) throw new Error("循环放映配置失败");
  if (action.operation === "setSpeakerNotes" && (!Array.isArray(operationData.updatedSlides) || operationData.updatedSlides.length !== 2)) throw new Error("批量备注写入失败");
  if (action.operation === "inspectSpeakerNotes" && Number(asRecord(operationData.summary).missingNotes) !== 0) throw new Error("备注检查发现缺失讲稿");
}
...
async function createFixture(presentationPath: string, templatePath: string, logoPath: string): Promise<void> {
