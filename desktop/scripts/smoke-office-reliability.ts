...
} from "../electron/agent/tools/officeCore/transactionJournal";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await createWorkbook(secondWorkbookPath, [
      ["名称", "值"],
      ["备用", 1],
    ]);
    ...
    const wordLinksBefore = linksFrom(
      await runAction(bridge, {
        app: "word",
        action: "inspect",
        operation: "inspectLinkedOfficeContent",
        filePath: linkedWordPath,
      }),
    );
    const presentationLinksBefore = linksFrom(
      await runAction(bridge, {
        app: "presentation",
        action: "inspect",
        operation: "inspectLinkedOfficeContent",
        filePath: linkedPresentationPath,
      }),
    );
    ...
    await runAction(bridge, {
      ...
      filePath: linkedWordPath,
      params: {
        edits: [{ command: "replaceBookmark", name: "ManualKeep", text: "人工保留段落" }],
        keepTracking: false,
      },
    });
    assertWordBookmark(
    ...
    const managedShapeName = String(
      presentationLinksBefore[0]?.shapeName || presentationLinksBefore[0]?.name || "",
    );
    if (!managedShapeName)
      throw new Error(`PowerPoint 链接缺少 shapeName: ${JSON.stringify(presentationLinksBefore)}`);
    await runAction(bridge, {
      app: "presentation",
      action: "style",
      ...
      params: {
        mode: "precise",
        shapeNames: [managedShapeName],
        edits: [
          {
            shapeName: managedShapeName,
            left: 96,
            top: 112,
            width: 410,
            height: 236,
    ...
    const managedLayoutBefore = findPresentationShape(
      await runAction(bridge, {
        app: "presentation",
        action: "inspect",
    ...
    const wordLinksAfterIncremental = linksFrom(
      await runAction(bridge, {
        app: "word",
        action: "inspect",
        operation: "inspectLinkedOfficeContent",
    ...
    if (
      wordLinksAfterIncremental.length !== 1 ||
      String(wordLinksAfterIncremental[0].linkId) !== wordLinkId
    ) {
      throw new Error(
        `Word 增量更新后的链接清单异常: ${JSON.stringify({ links: wordLinksAfterIncremental, update: operationData(wordIncremental) })}`,
      );
    }
    ...
    assertPresentationLayout(
      managedLayoutBefore,
      findPresentationShape(presentationAfterIncremental, managedShapeName),
    );
    ...
    const pipelineSteps: OfficeActionInput[] = [
      {
        ...
        operation: "formatChart",
        filePath: sourcePath,
        params: { chartIndex: 1, name: "RevenueChart", title: "流水线部门金额" },
      },
      {
        app: "excel",
        action: "insert",
        operation: "exportRangeToWord",
        filePath: sourcePath,
        outputPath: pipelineWordPath,
        target: "range:Sheet1!A1:B4",
        params: { linked: true, overwrite: true },
      },
      {
        app: "excel",
        action: "insert",
        operation: "exportRangeToPresentation",
        filePath: sourcePath,
        outputPath: pipelinePresentationPath,
        params: { linked: true, sourceType: "chart", chartName: "RevenueChart", overwrite: true },
      },
    ];
    const pipeline = await runOfficeWorkflow(bridge, pipelineSteps, {
      workflowRoot,
      transactionRoot,
    });
    if (pipeline.status !== "done" || !pipeline.transactionId)
      throw new Error(`办公流水线失败: ${pipeline.error || pipeline.status}`);
    ...
    if (transaction.artifacts.length !== 2 || transaction.changes.length < 3)
      throw new Error("事务产物或修改清单不完整");
    ...
    const resumableSteps: OfficeActionInput[] = [
      { app: "excel", action: "inspect", operation: "inspectCharts", filePath: sourcePath },
      {
        ...
        action: "inspect",
        operation: "inspectLinkedOfficeContent",
        filePath: pipelineWordPath,
      },
      {
        app: "presentation",
        action: "inspect",
        operation: "inspectLinkedOfficeContent",
        filePath: pipelinePresentationPath,
      },
    ];
    const paused = await runOfficeWorkflow(unstableBridge, resumableSteps, {
      workflowRoot,
      transactionRoot,
    });
    if (
      paused.status !== "paused" ||
      paused.completedSteps !== 1 ||
      paused.nextStep !== 2 ||
      !paused.workflowId
    ) {
    ...
    if (resumed.status !== "done" || resumed.completedSteps !== 3)
      throw new Error(`工作流续跑失败: ${JSON.stringify(resumed)}`);
    const savedWorkflow = await getOfficeWorkflow(workflowRoot, paused.workflowId);
    if (savedWorkflow.stepRecords.some((step) => step.status !== "done"))
      throw new Error("工作流步骤记录不完整");

    ownedProcesses = await openFixtures(
      [sourcePath, secondWorkbookPath],
      [selectionWordPath, linkedWordPath],
      [selectionPresentationPath, linkedPresentationPath],
    );
    const documentBridge = new OfficeDocumentComBridge();
    const documents = await documentBridge.listDocuments();
    const excelDocuments = documents.filter(
      (document) =>
        document.app === "excel" &&
        [sourcePath, secondWorkbookPath].some((file) => samePath(file, document.fullName)),
    );
    if (excelDocuments.length !== 2)
      throw new Error(`多工作簿识别失败: ${JSON.stringify(documents)}`);
    assertIndependentInstances(excelDocuments, "Excel");
    const wordDocuments = documents.filter(
      (document) =>
        document.app === "word" &&
        [selectionWordPath, linkedWordPath].some((file) => samePath(file, document.fullName)),
    );
    const presentationDocuments = documents.filter(
      (document) =>
        document.app === "presentation" &&
        [selectionPresentationPath, linkedPresentationPath].some((file) =>
          samePath(file, document.fullName),
        ),
    );
    if (wordDocuments.length !== 2)
      throw new Error(`多 Word 实例识别失败: ${JSON.stringify(documents)}`);
    if (presentationDocuments.length !== 2)
      throw new Error(`多 PowerPoint 实例识别失败: ${JSON.stringify(documents)}`);
    assertIndependentInstances(wordDocuments, "Word");
    assertIndependentInstances(presentationDocuments, "PowerPoint");
    const sourceDocument = excelDocuments.find((document) =>
      samePath(sourcePath, document.fullName),
    )!;
    const wordDocument = wordDocuments.find((document) =>
      samePath(selectionWordPath, document.fullName),
    )!;
    const presentationDocument = presentationDocuments.find((document) =>
      samePath(selectionPresentationPath, document.fullName),
    )!;
    const excelObjects = await documentBridge.listObjects({
      app: "excel",
      filePath: sourcePath,
      instanceId: sourceDocument.instanceId,
    });
    const chart = excelObjects.find(
      (item) => item.kind === "chart" && item.name === "RevenueChart",
    );
    if (!chart) throw new Error(`Excel 图表对象识别失败: ${JSON.stringify(excelObjects)}`);
    await documentBridge.activateObject({
      app: "excel",
      filePath: sourcePath,
      instanceId: sourceDocument.instanceId,
      locator: chart.locator,
    });
    const wordObjects = await documentBridge.listObjects({
      app: "word",
    ...
    await documentBridge.activateObject({
      ...
      filePath: selectionWordPath,
      instanceId: wordDocument.instanceId,
      locator: wordPage.locator,
    });
    const presentationObjects = await documentBridge.listObjects({
      app: "presentation",
      filePath: selectionPresentationPath,
      instanceId: presentationDocument.instanceId,
    });
    const selectedSlide = presentationObjects.find((item) => item.kind === "slide");
    const presentationShape = presentationObjects.find(
      (item) => item.kind === "shape" && item.parent === selectedSlide?.locator,
    );
    if (!presentationShape)
      throw new Error(`PowerPoint 对象识别失败: ${JSON.stringify(presentationObjects)}`);
    await documentBridge.activateObject({
      app: "presentation",
      filePath: selectionPresentationPath,
      instanceId: presentationDocument.instanceId,
      locator: presentationShape.locator,
    });
    ...
    summary = {
      ...
      pipeline: {
        ...
        artifacts: transaction.artifacts.length,
      },
      ...
      selection: {
        ...
        instances: {
          excel: excelDocuments.length,
          word: wordDocuments.length,
          presentation: presentationDocuments.length,
        },
        excelObjects: excelObjects.length,
        wordObjects: wordObjects.length,
        presentationObjects: presentationObjects.length,
      },
    };
  } catch (error) {
    primaryError = error;
    ...
    await cleanup(() =>
      assertProcessesStillRunning(protectedWpsProcesses, "测试前已存在的 WPS 窗口"),
    );
    await cleanup(() =>
      assertProcessesStillRunning(officeProcessesBefore, "测试前已存在的 Microsoft Office 进程"),
    );
    if (ownedProcesses)
      await cleanup(() => assertOwnedMicrosoftOfficeProcessesStopped(ownedProcesses!));
    ...
    if (keepArtifacts) process.stdout.write(`Office reliability smoke artifacts: ${tempDir}\n`);
      ...
      await cleanup(() =>
        rm(tempDir, { recursive: true, force: true, maxRetries: 5, retryDelay: 300 }),
      );
    await cleanup(() => disposeOfficeWorker());
...
async function createWorkbook(filePath: string, values: unknown[][]): Promise<void> {
  const result = await applyExcelAdvancedAction({
    ...
    filePath,
    params: { sheetNames: ["Sheet1"], values },
  });
...
async function runAction(
  ...
  if (result.status !== "done")
    throw new Error(`${input.app}/${input.operation}: ${result.error || result.summary}`);
...
function assertLinks(
  ...
  if (
    ...
    !links.some((link) =>
      String(link.source || "")
        .toLowerCase()
        .includes(path.basename(sourcePath).toLowerCase()),
    )
  ) {
...
function assertSameLocators(
  ...
  if (JSON.stringify(beforeLocators) !== JSON.stringify(afterLocators))
    throw new Error(`${label} 刷新后对象定位发生变化`);
}
...
function findPresentationShape(result: OfficeActionResult, name: string): Record<string, unknown> {
  ...
  const slides = (Array.isArray(rawSlides) ? rawSlides : rawSlides ? [rawSlides] : []).map(
    asRecord,
  );
...
function assertPresentationLayout(
...
async function assertTargetedRelink(
  ...
  if (
    !movedLink ||
    !String(movedLink.source || "")
      .toLowerCase()
      .includes(path.basename(temporarySource).toLowerCase())
  ) {
  ...
  if (Number(operationData(refreshed).updated) !== 1)
    throw new Error(`${app} 定向刷新影响了多个链接对象`);
  await runAction(bridge, {
    app,
    action: "edit",
    operation: "relinkLinkedOfficeContent",
...
async function verifyDirtyDocumentTransaction(input: {
  ...
  const workflow = await runOfficeWorkflow(
    input.bridge,
    [
      {
        app: "word",
  ...
  await assertWordBookmarkInFile(
    input.bridge,
    input.filePath,
    instanceId,
    "DirtyBaseline",
  ...
  const restoreOptions = {
    ...
    restoreFiles: (files: Parameters<OfficeDocumentComBridge["restoreTransactionFiles"]>[0]) =>
      input.documentBridge.restoreTransactionFiles(files),
  };
  ...
  await runAction(input.bridge, {
    ...
    params: {
      instanceId,
      edits: [{ command: "replaceBookmark", name: "DirtyBaseline", text: "事务外修改" }],
      keepTracking: false,
    },
  });
  const conflicted = await undoOfficeTransaction(
    input.transactionRoot,
    workflow.transactionId,
    restoreOptions,
  );
  ...
  await undoOfficeTransaction(input.transactionRoot, workflow.transactionId, {
    ...restoreOptions,
    force: true,
  });
  instanceId = await currentDocumentInstanceId(input.documentBridge, "word", input.filePath);
...
async function markOpenWordBookmarkDirty(
  filePath: string,
  instanceId: string,
  name: string,
  text: string,
...
async function currentDocumentInstanceId(
  ...
  const matches = (await bridge.listDocuments(app)).filter((document) =>
    samePath(filePath, document.fullName),
  );
  if (matches.length !== 1)
    throw new Error(`恢复后无法唯一定位 ${app} 文档: ${JSON.stringify(matches)}`);
...
async function waitForOfficeProcesses(
  selectPending: (processIds: number[]) => number[],
): Promise<number[]> {
  let pending: number[] = [];
...
function samePath(expected: string, actual?: string): boolean {
  return (
    Boolean(actual) && path.resolve(expected).toLowerCase() === path.resolve(actual!).toLowerCase()
  );
}
...
function assertIndependentInstances(
  documents: Array<{ processId?: number; instanceId: string }>,
  label: string,
): void {
  const instances = new Set(documents.map((document) => document.instanceId));
  const knownProcessIds = documents
    .map((document) => document.processId)
    .filter((id): id is number => Boolean(id));
  if (
    instances.size !== documents.length ||
    new Set(knownProcessIds).size !== knownProcessIds.length
  ) {
