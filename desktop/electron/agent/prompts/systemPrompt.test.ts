...
describe("buildContextualPromptSections", () => {
  test("injects formula assistant rules only for formula-writing turns", () => {
    ...
    expect(prompt).toContain("formula.prepare");
    expect(prompt).toContain("参考样例为空时");
    expect(prompt).toContain("方法论不区分难度且必须成功");
    expect(prompt).toContain("no_match/unavailable");
    expect(prompt).toContain("最小充分结构");
    expect(prompt).toContain("生成业务键、条件或索引");
    expect(prompt).toContain("环境支持且模式稳定");
    expect(prompt).toContain("自动回读和 `formula.verify`");
    expect(prompt).toContain("ASCII");
    expect(prompt).toContain("WPS 报公式错误");
    expect(prompt).toContain("动态数组只写锚点");
