...
describe("DotNetOfficeDocumentBridge", () => {
  it("forwards document and object selection routes", async () => {
    ...
    expect(invoke).toHaveBeenNthCalledWith(
      2,
      "office.documents.activate",
      expect.objectContaining({ instanceId: "word:1:2" }),
    );
    expect(invoke).toHaveBeenNthCalledWith(
      3,
      "office.objects.list",
      expect.objectContaining({ kind: "shape" }),
    );
    expect(invoke).toHaveBeenNthCalledWith(
      4,
      "office.objects.activate",
      expect.objectContaining({ locator: "sheet:Data" }),
    );
  });
  ...
  it("forwards transaction preparation and atomic restore routes", async () => {
    ...
    expect(invoke).toHaveBeenNthCalledWith(1, "office.transaction.prepare", {
      filePaths: ["C:\\book.xlsx"],
    });
