...
/**
 * 处理流式中断（AbortError）时保存当前进度
 *
 * 在 collectStreamEvents 抛出 AbortError 后调用，
 * 将已收集的内容保存为 TurnItem 并通过事件发出。
 */
export async function emitInterruptedProgress(
  result: StreamResult,
  turnItems: TurnItem[],
  threadId: string,
  turnId: string,
  ...
  if (result.reasoningContent.length > 0) {
    const reasoningItem: TurnItem = {
      ...
      summaryText: result.reasoningSummary,
      rawContent: result.reasoningContent,
      timestamp: Date.now(),
    };
