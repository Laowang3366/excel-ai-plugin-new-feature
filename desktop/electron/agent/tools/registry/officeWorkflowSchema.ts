...
const WORKFLOW_DATA_PATH = {
  ...
  maxLength: 2_079,
  pattern:
    "^(?!(?:__proto__|prototype|constructor)(?:\\.|$))[A-Za-z_][A-Za-z0-9_-]{0,63}(?:\\.(?!(?:__proto__|prototype|constructor)(?:\\.|$))(?:(?:0|[1-9][0-9]*)|[A-Za-z_][A-Za-z0-9_-]{0,63})){0,31}$",
};
...
export const OFFICE_WORKFLOW_VARIABLES_SCHEMA: Record<string, unknown> = {
  ...
  description:
    "模板变量，最多 128 个顶层键；变量路径最多 32 段，每段仅允许安全字母、数字、下划线或连字符。嵌套对象和数组可通过 {{vars.customer.name}}、{{vars.files.0}} 引用，只读取真实自有属性。",
};
  ...
  withOfficeOperationDiscriminator(
    {
      ...
      properties: {
        ...
        id: { ...WORKFLOW_SELECTOR, description: "可选稳定步骤 ID，供占位符和条件引用" },
        parallelGroup: {
          ...WORKFLOW_SELECTOR,
          description: "连续且同名的步骤并行执行；写入目标不得重复",
        },
        timeoutMs: {
          type: "integer",
          minimum: 5_000,
        ...
        when: {
          ...
          properties: {
            step: {
              oneOf: [{ type: "integer", minimum: 1 }, WORKFLOW_SELECTOR],
              description: "依赖步骤序号或 ID",
            },
            status: { type: "string", enum: ["done", "failed", "skipped"] },
            dataPath: { ...WORKFLOW_DATA_PATH, description: "可选结果 data 路径" },
