  ...
  parameters: {
    ...
    properties: {
      filePaths: {
        ...
        minItems: 1,
        description:
          "要识别的本地文件绝对路径列表，支持 PNG/JPG/JPEG/WebP/BMP/TIFF/PDF/DOC/DOCX/PPT/PPTX/XLS/XLSX/XLSM/CSV/MD/TXT",
        items: { type: "string" },
      },
      mode: {
        type: "string",
        ...
        description:
          "解析意图。ocr 为通用文字/表格识别；invoice 用于票据字段抽取前置识别；layout 用于可见结构和布局诊断；style 用于样式状态评估或修改后验收。工具返回解析文本供模型继续判断",
        default: "ocr",
      },
      maxTextChars: {
        type: "integer",
      ...
      allowTokenMineru: {
        type: "boolean",
        description:
          "是否允许优先使用已配置 token 的 MinerU 标准解析，默认 true。只有用户明确要求不消耗 token 时才设为 false",
        default: true,
      },
      allowFreeMineru: {
        type: "boolean",
...
export const OCR_TOOL_DEFINITIONS: ToolDefinition[] = [OCR_PARSE_DOCUMENT_DEF];
