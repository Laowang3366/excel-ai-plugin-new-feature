...
const KNOWLEDGE_SEARCH_DEF: ToolDefinition = {
  ...
  description:
    "搜索本地知识库中的项目资料、业务口径、模板规范和历史规则。先读取当前文件或数据，仅在任务依赖已沉淀知识或用户明确要求时调用；返回片段用于补充当前文件事实，不得把最相似案例直接当作答案",
  parameters: {
    type: "object",
    properties: {
      query: {
        type: "string",
        description: "结构化搜索词，包含任务、业务对象、关键字段、约束和目标；样例值只作补充",
      },
      topK: {
        type: "integer",
        minimum: 1,
        maximum: 50,
        description: "返回结果数量，默认5，最多50",
        default: 5,
      },
    },
    required: ["query"],
...
const KNOWLEDGE_WRITE_DEF: ToolDefinition = {
  ...
  description:
    "把用户明确要求保存为知识库的内容写入本地知识库，供后续 knowledge.search 检索。仅在用户说“写入知识库”“记到知识库”“保存为知识”等明确触发时使用，不要替代 memory.write",
  parameters: {
    type: "object",
    properties: {
      content: {
        type: "string",
        description: "要写入知识库的正文，必须是用户明确希望沉淀的知识内容",
      },
...
const KNOWLEDGE_LIST_SOURCES_DEF: ToolDefinition = {
  ...
  description:
    "列出已经索引到本地知识库的来源文件，用于在修改、追加或删除知识库内容前确认 sourcePath。普通检索仍优先使用 knowledge.search。",
  parameters: {
    type: "object",
    properties: {},
  },
...
const KNOWLEDGE_UPDATE_SOURCE_DEF: ToolDefinition = {
  ...
  parameters: {
    ...
    properties: {
      sourcePath: {
        type: "string",
        description: "要修改的已索引知识来源绝对路径，通常来自 knowledge.listSources 的返回值。",
      },
      operation: {
        type: "string",
        enum: ["replace", "append"],
      ...
      title: {
        type: "string",
        description: "可选标题；replace 写入 Markdown 来源时会作为标题使用。",
      },
      tags: {
        type: "array",
        description: "可选标签。",
  ...
  parameters: {
    ...
    properties: {
      sourcePath: {
        type: "string",
        description:
          "要从知识库索引中删除的来源绝对路径，通常来自 knowledge.listSources 的返回值。",
      },
    },
    required: ["sourcePath"],
  },
