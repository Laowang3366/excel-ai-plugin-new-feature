...
const KNOWLEDGE_SEARCH_DEF: ToolDefinition = {
  ...
  description: "搜索本地知识库，获取与当前任务相关的方法论、业务口径、公式规则、模板规范和历史资料。先读取当前文件/数据并判断场景难度；简单问答、单步操作无需调用，中高复杂度任务才按输入/输出形状、业务键、约束和所需变换检索。返回片段用于归纳解题链和校验局部规则，不得把最相似案例直接当作答案",
  parameters: {
    type: "object",
    properties: {
