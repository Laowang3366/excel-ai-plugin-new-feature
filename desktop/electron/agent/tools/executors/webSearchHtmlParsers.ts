...
export function parseDuckDuckGoHtml(html: string): WebSearchResultItem[] {
  ...
  while ((match = resultRe.exec(html))) {
    const block = match[0];
    const link =
      /<a[^>]+class="[^"]*\bresult__a\b[^"]*"[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/i.exec(block);
    if (!link) continue;
    const snippet =
...
export function parseBingHtml(html: string): WebSearchResultItem[] {
  ...
  while ((match = resultRe.exec(html))) {
    const block = match[0];
    const link =
      /<h2[^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h2>/i.exec(
        block,
...
export function parseBaiduHtml(html: string): WebSearchResultItem[] {
  const results: WebSearchResultItem[] = [];
  const resultRe =
    /<div[^>]+class=["'][^"']*(?:result|c-container)[^"']*["'][^>]*>[\s\S]*?(?=<div[^>]+class=["'][^"']*(?:result|c-container)|<\/body>|$)/gi;
  let match: RegExpExecArray | null;
  while ((match = resultRe.exec(html))) {
    const block = match[0];
    const link =
      /<h3[^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h3>/i.exec(
        block,
      ) || /<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/i.exec(block);
    if (!link) continue;
    ...
    if (!url || !/^https?:\/\//i.test(url)) continue;
    const snippet =
      /<div[^>]+class=["'][^"']*(?:c-abstract|content-right|c-span-last)[^"']*["'][^>]*>([\s\S]*?)<\/div>/i.exec(
        block,
      ) ||
      /<span[^>]+class=["'][^"']*(?:content-right|c-color-text)[^"']*["'][^>]*>([\s\S]*?)<\/span>/i.exec(
        block,
      );
    results.push({
      title: cleanText(decodeHtml(stripTags(link[2]))),
...
export function parseSoHtml(html: string): WebSearchResultItem[] {
  const results: WebSearchResultItem[] = [];
  const resultRe =
    /<li[^>]+class=["'][^"']*\bres-list\b[^"']*["'][^>]*>[\s\S]*?(?=<li[^>]+class=["'][^"']*\bres-list\b|<\/ul>|$)/gi;
  let match: RegExpExecArray | null;
  while ((match = resultRe.exec(html))) {
    const block = match[0];
    const link =
      /<h3[^>]+class=["'][^"']*\bres-title\b[^"']*["'][^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h3>/i.exec(
        block,
      ) ||
      /<h3[^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h3>/i.exec(
        block,
    ...
    if (!url || !/^https?:\/\//i.test(url)) continue;
    const snippet =
      /<p[^>]+class=["'][^"']*\bres-desc\b[^"']*["'][^>]*>([\s\S]*?)<\/p>/i.exec(block) ||
      /<span[^>]+class=["'][^"']*\bres-list-summary\b[^"']*["'][^>]*>([\s\S]*?)<\/span>/i.exec(
        block,
      ) ||
...
export function parseSogouHtml(html: string): WebSearchResultItem[] {
  const results: WebSearchResultItem[] = [];
  const resultRe =
    /<div[^>]+class=["'][^"']*\bvrwrap\b[^"']*["'][^>]*>[\s\S]*?(?=<div[^>]+class=["'][^"']*\bvrwrap\b|<\/body>|$)/gi;
  let match: RegExpExecArray | null;
  while ((match = resultRe.exec(html))) {
    const block = match[0];
    const link =
      /<h3[^>]+class=["'][^"']*(?:vr-title|vrTitle)[^"']*["'][^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h3>/i.exec(
        block,
      ) ||
      /<h3[^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/h3>/i.exec(
        block,
    ...
    if (!url || !/^https?:\/\//i.test(url)) continue;
    const snippet =
      /<div[^>]+class=["'][^"']*(?:fz-mid|space-txt|base-ellipsis|text-layout)[^"']*["'][^>]*>([\s\S]*?)<\/div>/i.exec(
        block,
      ) ||
