  ...
  it("rolls back all writes when a transaction fails", async () => {
    ...
    await expect(
      store.transaction(async (tx) => {
        await tx.upsertThreadSnapshot(createMetadata());
        throw new Error("boom");
      })
    ).rejects.toThrow("boom");

    expect(await store.getThreadSnapshot("thread-1")).toBeNull();
    await store.close();
  });

  it("does not persist rolled back transaction writes to disk", async () => {
    const tempDir = await mkdtemp(path.join(os.tmpdir(), "state-runtime-"));
    try {
      const store = new StateRuntimeStore(tempDir);
      await store.init();
      ...
      expect(await reloaded.getThreadSnapshot("thread-1")).toBeNull();
      expect(await reloaded.getThreadRuntime("thread-1")).toBeNull();
      await reloaded.close();
    } finally {
      await rm(tempDir, { recursive: true, force: true });
    }
