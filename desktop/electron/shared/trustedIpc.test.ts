...
describe("trusted IPC sender policy", () => {
  it("accepts the production file renderer and exact development origin", () => {
    ...
    expect(isTrustedRendererUrl("http://localhost:5173/chat", "http://localhost:5173")).toBe(true);
  });
  ...
  it("requires the current main window and its top-level frame", () => {
    ...
    expect(
      isTrustedIpcSender(trusted.event as never, trusted.getMainWindow as never, undefined),
    ).toBe(true);

    const subframe = createSender("file:///C:/app/dist/index.html", false);
    expect(
      isTrustedIpcSender(subframe.event as never, subframe.getMainWindow as never, undefined),
    ).toBe(false);

    const remote = createSender("https://attacker.example");
    expect(
      isTrustedIpcSender(remote.event as never, remote.getMainWindow as never, undefined),
    ).toBe(false);
  });
});
