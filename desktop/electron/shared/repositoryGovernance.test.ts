...
describe("repository governance baseline", () => {
  ...
  it("keeps the data map factual about remote processing and deletion gaps", () => {
    ...
    expect(dataHandling).toContain("已登记活动根、迁移旧根与应用创建的隐私导出副本");
    expect(dataHandling).toContain("replacement");
    expect(dataHandling).toContain("当前数据根路径会保留");
    expect(dataHandling).toContain("未登记");
    expect(dataHandling).toContain("删除证明");
