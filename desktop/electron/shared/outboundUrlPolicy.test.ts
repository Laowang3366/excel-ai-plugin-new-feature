...
describe("outbound URL policy", () => {
  ...
  it("allows public HTTPS endpoints", async () => {
    await expect(
      validateOutboundUrl("https://api.example.com/v1", {
  ...
  it("rejects credentials, private hosts, metadata and encoded IP forms", async () => {
    ...
    await expect(validateOutboundUrl("https://127.0.0.1")).rejects.toThrow(
      "outbound_private_address_forbidden",
    );
    await expect(validateOutboundUrl("https://2130706433")).rejects.toThrow(
      "outbound_private_address_forbidden",
    );
    await expect(validateOutboundUrl("https://[::1]")).rejects.toThrow(
      "outbound_private_address_forbidden",
    );
    await expect(validateOutboundUrl("https://metadata.google.internal")).rejects.toThrow(
      "outbound_private_host_forbidden",
    );
  });

  it("checks every DNS answer and allows only explicitly configured local origins", async () => {
    await expect(
      validateOutboundUrl("https://rebind.example", {
        lookupAll: async () => [
    ...
    await expect(
      validateOutboundUrl("http://127.0.0.1:11434/v1", {
