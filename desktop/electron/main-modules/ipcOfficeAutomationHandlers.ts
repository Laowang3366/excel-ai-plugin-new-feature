...
} from "../agent/tools/contracts/office";
...
} from "../agent/tools/officeCore/transactionJournal";
import {
  getOfficeWorkflow,
  listOfficeWorkflows,
  requestOfficeWorkflowCancellation,
  runOfficeWorkflow,
} from "../agent/tools/officeCore/workflow";
import {
...
export function createOfficeAutomationService(deps: OfficeAutomationServiceDeps) {
  ...
  const actionBridge = (transactionRoot: string) =>
    deps.createActionBridge?.(transactionRoot, documentBridge) ||
  ...
  const restoreOptions = (force = false) => ({
    ...
    restoreFiles: (files: Parameters<OfficeDocumentManagerBridge["restoreTransactionFiles"]>[0]) =>
      documentBridge.restoreTransactionFiles(files),
  });

  return {
    listDocuments: (app?: OfficeActionApp) => documentBridge.listDocuments(app),
    activateDocument: (input: { app: OfficeActionApp; filePath: string; instanceId?: string }) =>
      documentBridge.activateDocument(input),
    listObjects: (input: {
      app: OfficeActionApp;
      filePath: string;
      instanceId?: string;
      kind?: string;
    }) => documentBridge.listObjects(input),
    activateObject: (input: {
      app: OfficeActionApp;
      filePath: string;
      instanceId?: string;
      locator: string;
    }) => documentBridge.activateObject(input),
    ...
    saveTemplateFromWorkflow: async (input: {
      workflowId: string;
      templateId?: string;
      name: string;
      description?: string;
      ...
      return saveOfficeWorkflowTemplate({
        root: workflowRoot,
        id: input.templateId,
        name: input.name,
        description: input.description,
        steps: workflow.sourceSteps || workflow.steps,
      });
    },
    ...
    undoTransaction: (id: string, force = false) =>
      undoOfficeTransaction(roots().transactionRoot, id, restoreOptions(force)),
    redoTransaction: (id: string, force = false) =>
      redoOfficeTransaction(
        roots().transactionRoot,
        id,
        actionBridge(roots().transactionRoot),
...
export function registerOfficeAutomationIpcHandlers(deps: OfficeAutomationServiceDeps): void {
  ...
  handle("office:automation:documents:list", (input: unknown) =>
    service.listDocuments(validateInput(OfficeAutomationDocumentsListInput, input)?.app),
  );
  handle("office:automation:documents:activate", (input: unknown) =>
    service.activateDocument(validateInput(OfficeAutomationDocumentInput, input)),
  );
  handle("office:automation:objects:list", (input: unknown) =>
    service.listObjects(validateInput(OfficeAutomationObjectsListInput, input)),
  );
  handle("office:automation:objects:activate", (input: unknown) =>
    service.activateObject(validateInput(OfficeAutomationObjectActivateInput, input)),
  );
  handle("office:automation:workflows:list", () => service.listWorkflows());
  handle("office:automation:workflows:get", (input: unknown) =>
    service.getWorkflow(validateInput(OfficeAutomationIdInput, input).id),
  );
  handle("office:automation:workflows:cancel", (input: unknown) =>
    service.cancelWorkflow(validateInput(OfficeAutomationIdInput, input).id),
  );
  handle("office:automation:workflows:resume", (input: unknown) =>
    service.resumeWorkflow(validateInput(OfficeAutomationIdInput, input).id),
  );
  handle("office:automation:templates:list", () => service.listTemplates());
  handle("office:automation:templates:saveFromWorkflow", (input: unknown) =>
    service.saveTemplateFromWorkflow(validateInput(OfficeAutomationTemplateSaveInput, input)),
  );
  handle("office:automation:templates:delete", (input: unknown) =>
    service.deleteTemplate(validateInput(OfficeAutomationIdInput, input).id),
  );
  handle("office:automation:templates:run", (input: unknown) => {
    const value = validateInput(OfficeAutomationTemplateRunInput, input);
    return service.runTemplate(value.templateId, value.variables);
  });
  handle("office:automation:transactions:list", () => service.listTransactions());
  handle("office:automation:transactions:get", (input: unknown) =>
    service.getTransaction(validateInput(OfficeAutomationIdInput, input).id),
  );
  handle("office:automation:transactions:undo", (input: unknown) => {
    const value = validateInput(OfficeAutomationForceInput, input);
    return service.undoTransaction(value.id, value.force);
  });
  handle("office:automation:transactions:redo", (input: unknown) => {
    const value = validateInput(OfficeAutomationForceInput, input);
    return service.redoTransaction(value.id, value.force);
  });
}
