...
describe("window navigation policy", () => {
  it("allows only the configured application document", () => {
    ...
    expect(isAllowedWindowNavigation("file:///C:/Windows/System32/drivers/etc/hosts", appUrl)).toBe(
      false,
    );
  ...
  it("does not trust another host that resembles the development origin", () => {
    ...
    expect(isAllowedWindowNavigation("http://localhost.attacker.example:5173/", appUrl)).toBe(
      false,
    );
  });
});
