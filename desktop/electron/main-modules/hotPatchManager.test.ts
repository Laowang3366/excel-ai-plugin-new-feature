...
describe("installHotPatchArchive", () => {
  it("installs and activates a verified patch atomically", async () => {
    ...
    expect(await fs.readFile(path.join(state.rootPath, "dist/index.html"), "utf8")).toContain(
      "patched",
    );
    expect(activateInstalledHotPatch("0.1.79", userDataPath)).toBe("patch-001");
    expect(resolveHotPatchPath("dist/index.html")).toBe(
      path.join(state.rootPath, "dist/index.html"),
    );
  });
  ...
  it("rejects renderer patches without the required entry point", async () => {
    const archive = await createArchive({ "dist/assets/app.js": "incomplete" });
    ...
    await expect(
      installHotPatchArchive({
        ...
        currentVersion: "0.1.79",
        userDataPath: path.join(archive.root, "user-data"),
        descriptor: descriptor(archive, "patch-missing-entry"),
      }),
    ).rejects.toThrow("缺少 dist/index.html");
  });
