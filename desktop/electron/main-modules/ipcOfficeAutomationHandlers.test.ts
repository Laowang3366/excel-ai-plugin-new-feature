...
describe("Office automation IPC service", () => {
  ...
  it("lists and activates documents and objects through the shared document bridge", async () => {
    ...
    expect(documentBridge.activateDocument).toHaveBeenCalledWith(
      expect.objectContaining({ instanceId: "excel:1" }),
    );
    expect(documentBridge.activateObject).toHaveBeenCalledWith(
      expect.objectContaining({ locator: "sheet:Sheet1" }),
    );
  });
  ...
  it("saves a template only from a persisted workflow and runs it with variables", async () => {
    ...
    const first = await runOfficeWorkflow(
      actionBridge,
    ...
    const template = await service.saveTemplateFromWorkflow({
    ...
    expect(executeAction).toHaveBeenLastCalledWith(
      expect.objectContaining({ filePath: sourcePath, transactionContext: "workflow" }),
    );
  });
});
...
function createDocumentBridge(): OfficeDocumentManagerBridge {
  return {
    ...
    activateDocument: vi.fn(async (input) => ({
      ...
      instanceId: input.instanceId || "excel:1",
    })),
