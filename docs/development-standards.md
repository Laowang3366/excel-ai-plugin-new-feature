...
| 文件类型 | 上限 | 超限后果 |
|----------|------|----------|
| TypeScript/TSX 通用**生产**模块 | ≤ 400 行 | 必须按职责拆分后才能合入 |
| React 组件 | ≤ 300 行 | 提取子组件 + hooks |
| Zustand Store | ≤ 400 行 | 提取 actions/事件处理为独立模块 |
| CSS 文件 | ≤ 500 行 | 按组件/功能域拆分 |

行数上限针对生产模块与 UI/Store/CSS。`desktop/scripts/smoke-*.ts` 真实 Office/WPS 冒烟场景脚本是线性验收步骤，不因超过 400 行强制拆分；仅当出现可复用 helper 时再提取。冒烟脚本仍受 Prettier 增量棘轮约束。
...
- 任一存量超限**生产**文件被修改后必须按职责拆到本节上限以内；新生产文件从首次提交起直接执行上限；
- `desktop/scripts/smoke-*.ts` 不进入行数 oversized 扫描（仅限该目录下、文件名匹配的冒烟脚本），但仍参与 Prettier 漂移棘轮；
