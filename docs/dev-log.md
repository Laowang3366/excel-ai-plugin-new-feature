...
## 2026-07-13 - Office 高级自动化与 WPS 演示兼容

- 完成 Excel 打印、公式治理、Power Query、图表、工作簿对象和模板能力，Word 长文档、引用、修订、邮件合并和内容控件能力，以及 PowerPoint/WPS 母版品牌、元素排版、动画放映、备注和讲义 PDF。
- Excel 区域和图表可作为链接 OLE 写入 Word/PPT；数据变化后原位刷新，不重建人工调整过的页面对象。
- 新增持久化 Office 流水线：每步记录产物和操作结果，失败后从当前步骤继续；组事务可查看修改清单、整体撤销和重做。
- 多窗口定位扩展到对象级：按完整文件路径列出并激活工作表、区域、图表、Word 页面/书签和 PPT 幻灯片/形状，避免同名文件或后台窗口误操作。
- Office COM 生命周期按进程归属清理：仅退出本次新建的 Office 进程；复用用户已有 WPS 进程时只关闭本次打开的文件，避免误关其他文档。
- WPS 12.0 不支持 COM 写入主题色且可能只持久化首个备注页，改为在 WPS 保存关闭后使用结构化 Open XML 回退，并由 WPS 再次打开回读验收。
- 真实冒烟通过：Excel/跨 Office 33 步、Word 18 步、PowerPoint 15 步、WPS 演示 15 步；Word、Excel 同文件连续打开两次，WPS PPT 连续打开并取得独占写锁。
- Office 可靠性真实冒烟通过：Word/PPT 链接创建和刷新、3 步 Excel→Word→PPT 流水线、失败续跑、跨文件撤销重做，以及 4 个并行文档的对象选择；测试后未遗留 Microsoft Office 进程，原有 WPS 会话保持运行。
- 桌面端验证基线为 174 个测试文件、968 项测试；类型检查、ESLint 和生产构建通过。

## 2026-07-12 - 远程更新与产品站

- 新增 Ed25519 签名更新清单、安装包更新和受限热补丁，设置页可检查、下载并重启应用完成更新。
- 新增独立产品页、安装包分发、下载统计后台和隔离部署配置，服务监听 `127.0.0.1:18120`。
- 更新日志改为只记录用户可感知功能，发布资产通过 GitHub Release 与产品站提供，不写入 Git 大文件。
- 桌面端验证基线为 174 个测试文件、968 项测试；产品站测试独立执行。
