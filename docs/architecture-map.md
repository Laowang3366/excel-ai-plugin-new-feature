...
- Renderer 的 `electronApi.d.ts` 是类型门面；Agent/Thread 投影在 `electronApiAgentTypes.ts`，Update/Office/AI/文件夹等领域类型在 `electronApiDomainTypes.ts`，`ElectronAPI` 接口在 `electronApiInterface.ts`，既有 `from ../electronApi` 导入无需迁移。
...
flowchart TB
  ...
  subgraph Bridges["Electron 类型化桥接"]
    ...
    Transactions["officeCore/transactions.ts + transactionTypes.ts + transactionPaths.ts + transactionRecordStore.ts + transactionJournal.ts + workflowTypes.ts + workflowHelpers.ts + workflow.ts"]
...
| 高级对象与跨应用操作 | `office.action.*`、`office.workflow.*`、`office.transaction.*` | `officeActionTransactionAdapter.ts` 负责单动作备份和独立跨 Office 事务；`officeActionAdapter.ts` 负责 Open XML/COM 能力路由；`officeActionValidation.ts` 解释验证结果；`officeReliabilityExecutors.ts` -> `workflow.ts` / `workflowTypes.ts` / `workflowHelpers.ts` / `workflowRecordStore.ts` / `transactionJournal.ts` / `transactionTypes.ts` / `transactionPaths.ts` / `transactionRecordStore.ts` | Excel 查询/打印/公式治理、Word/PPT 高级编辑、链接报告原位刷新、步骤产物、暂停续跑、整体撤销和重做 |
