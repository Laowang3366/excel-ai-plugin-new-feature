...
| L-01 | 部分完成 | CI 哈希棘轮冻结存量债务；任务面板、聊天 Store、i18n 已按模型/类型化资源解耦；`App`/`ChatPage`/热补丁/OCR/Web 搜索/Agent IPC/设置 IPC/知识库/Office action/`excelExecutors`/`workflow`/`transactionJournal`/`ipcSchemas`/`shared/types` 已按职责拆分；Renderer `electronApi.d.ts` 已按 Agent 投影与领域类型拆分 | 继续分批格式化未触碰存量文件；剩余 oversized 以 governance 输出为准 |
...
| Desktop `format:check` | **失败** | 限定源码检查仍有 476 个存量格式不匹配文件；增量 `governance:check` 棘轮通过 |
