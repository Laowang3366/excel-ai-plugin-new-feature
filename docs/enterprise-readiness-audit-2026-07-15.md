...
| M-12 | 部分完成 | 产品站 SQLite 在线备份、SHA-256 元数据、完整性校验、安全恢复、14 份轮换与 timer；桌面日志/Office 备份/事务/工作流按 TTL、条目和字节配额周期清理并保护活动记录 | 生产负责人确认 RPO/RTO并完成恢复演练；异机副本、用户清理/导出、删除证明和全链路告警 |
...
**验收状态**：派生日志与索引的明文复制已关闭，单会话删除闭环已覆盖会话正文及冷归档；日志、Office 备份、事务与工作流已有自动 TTL 和容量上限。JSONL 和 `rollout_events.item_json` 在用户未删除会话时仍作为恢复/兼容数据保留。M-03 尚未关闭，后续需完成恢复数据应用层加密、密钥轮换、统一导出和跨会话/知识/记忆/备份的全局擦除策略。
