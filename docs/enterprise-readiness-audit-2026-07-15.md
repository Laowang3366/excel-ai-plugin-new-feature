...
| M-03 | 部分完成 | 统一高置信密钥与敏感字段脱敏；通用 logger 在控制台/文件前递归脱敏并限制单行；工具审计只保存结构统计与指纹；FTS 移除重复 `item_json`；单会话删除覆盖 JSONL、gzip/zstd 冷归档、SQLite/FTS/工具日志并保持失败一致性 | 会话/SQLite 应用层加密；日志与 Office 自动化数据留存、导出和全局擦除闭环 |
...
**验收状态**：派生日志与索引的明文复制已关闭，单会话删除闭环已覆盖会话正文及冷归档；JSONL 和 `rollout_events.item_json` 在用户未删除会话时仍作为恢复/兼容数据保留。M-03 尚未关闭，后续需完成恢复数据应用层加密、密钥轮换，以及 logs/Office 备份/事务/工作流的统一 TTL、导出和全局擦除策略。
